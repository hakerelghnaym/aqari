...
const Login = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) { toast.error("الرجاء إدخال البريد وكلمة السر"); return; }
    setBusy(true);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="bg-gradient-hero text-primary-foreground px-6 pt-10 pb-16 rounded-b-[2.5rem] relative overflow-hidden">
        ...
        <div className="relative">
          <div className="w-16 h-16 rounded-2xl bg-gradient-accent flex items-center justify-center mb-4 shadow-gold">
            <Building2 className="w-8 h-8 text-accent-foreground" />
          </div>
          <h1 className="text-2xl font-extrabold">تسجيل الدخول</h1>
          <p className="text-primary-foreground/80 mt-1.5 text-sm">أهلاً بعودتك! سجّل دخولك للمتابعة</p>
        </div>
      </div>

      <form onSubmit={onSubmit} className="flex-1 px-6 -mt-5 pb-10 space-y-4">
        <div className="bg-card rounded-3xl shadow-elevated p-5 space-y-4">
          <div className="space-y-2">
            <Label className="text-sm font-bold">البريد الإلكتروني</Label>
            <div className="relative">
              <Mail className="w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <Input value={email} onChange={e => setEmail(e.target.value)} type="email" inputMode="email"
                placeholder="example@mail.com" className="h-12 pr-11 text-base" />
            </div>
          </div>

          <div className="space-y-2">
            <Label className="text-sm font-bold">كلمة السر</Label>
            <div className="relative">
              <Lock className="w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <Input value={password} onChange={e => setPassword(e.target.value)}
                type={show ? "text" : "password"} placeholder="••••••••" className="h-12 pr-11 pl-11 text-base" />
        ...
        <p className="text-center text-sm text-muted-foreground pt-1">
          ليس لديك حساب?{" "}
          <Link to="/register" className="text-accent font-bold">إنشاء حساب جديد</Link>
        </p>
      </form>
    </div>
  );
